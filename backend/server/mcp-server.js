...
// Load .env only in non-production. Prefer server/.env (next to this file);
// cwd may be repo root so plain dotenv.config() misses it.
if (process.env.NODE_ENV !== 'production') {
  const dotenv = (await import('dotenv')).default
  dotenv.config({ path: path.join(__dirname, '.env') })
  if (!process.env.GOOGLE_API_KEY) {
    // fallback to repo-level .env if present
    dotenv.config({ path: path.join(__dirname, '..', '.env') })
  }
}
...
// ensure genAI is initialized safely (will be null if no API key)
